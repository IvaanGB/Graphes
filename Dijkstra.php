...
class Dijkstra {
    ...
    function getChemin_minimal() {
        return $this->chemin_minimal;
    }

    function getDistance_minimale() {
        return $this->distance_minimale;
    }

    function setGraphe(Graphe $g) {
        $this->graphe = $g;
    }
    ...
    function recherche() {
        if ($this->getDepart() === null) {
            echoln("le noeud de départ n'est pas précisé");
            return false;
        }
        if ($this->getArrivee() === null) {
            echoln("le noeud d'arrivé n'est pas précisé");
            return false;
        }
        $iteration = 0;
    ...
    public function get_string_chemin() {
        ...
        foreach ($this->chemin_minimal as $etape) {
            $path .= " <i class='fa fa-arrow-right'></i> <span class='badge'>" . $etape->getNom() . '</span>';
        }
        $path = substr($path, 35); // TODO à améliorer
        return $path;
    }
